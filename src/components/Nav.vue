...
<template>
  ...
  <nav v-if="authStore.getUser" class="navbar navbar-expand navbar-dark bg-dark">
    <div class="navbar-nav">
      <router-link to="/" class="nav-item nav-link">Home</router-link>

      <!-- Show the 'Usuários' link only if the user has an admin role -->
      <router-link
        v-if="authStore.getUser.tipos[0] === 'ROLE_ADMIN'"
        to="/usuarios"
      ...
      <router-link
        :to="`/usuarios/editar/${authStore.getUser.id}`"
        class="nav-item nav-link"
      >
        Meu Cadastro
      </router-link>
